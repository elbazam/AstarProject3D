    ...
    if idx < l:
        ...
        if d > thre_dist:
            ...
            if np.absolute(th_error) > threshold:
                if np.absolute(th_error) > np.pi and np.absolute(th_error) < (35/18)*np.pi:
                    th_error = -th_error
                publish_vel(0,np.sign(th_error)*threshold*0.5,pub)
            else:

                publish_vel(0.1,0,pub)
        elif np.abs(th_req) > threshold:
                if np.abs(th_req) > np.pi and np.absolute(th_req) < (35/18)*np.pi:
                    th_req = -th_req
                if th_req > 0.05: th_req = threshold * 0.5
                elif th_req < -0.05: th_req = -threshold * 0.5
                publish_vel(0,th_req,pub)
        else:
            publish_vel(0,0,pub)
            idx = idx + 1
            if idx < l:
